...
def checkLocalGQStatusMissmatch(config):
    """
    check for LQ WQE in Available (acquired for inbox) and GQE in acquired
    """
    backend = WorkQueueBackend(config.WorkQueueManager.couchurl)
    lqStatus = "Available"
    gqStatus = "Acquired"
    ...
    for db in ("workqueue", "workqueue_inbox"):
        elementsWQ = backend.getElements(status=lqStatus)

        for elem in elementsWQ:
            print("Analyzing element: %s" % elem.id)
            if elem['Status'] == 'Available':
                elemInbox = backend.getInboxElements(elementIDs=[elem.id])[0]
                if elemInbox['Status'] == gqStatus:
                    print("Matching criteria:\nRequestName: %s\nWQElement: %s" % (elem['RequestName'], elem.id))
    return
...
def getWMBSInfo(config):
    ...
    checkLocalGQStatusMissmatch(config)
